...
\begin{dfn}
The \textbf{distribution} of random variable $X$ is the collection of all probabilities $\Pr(X \in C)$ for all sets $C$ of real numbers such that $X \in C$ is an event. 
\end{dfn}
...
\begin{Exercise}
Verify that the Bernoulli pmf $f(x) = \pi^x (1 - \pi)^{(1 - x)}$ for $x \in \{0, 1\}$ (and 0 otherwise) is a pmf. This requires two things. First, show that $f(x) \geq 0$ for $x \in \mathbb{R}$. Second, show that $\sum_{\text{support of }X} f(x) = 1$.
\end{Exercise}
...
We can use the `dlogis()` function in R (i.e., the pdf) to plot the pdf of the standard logistic distribution.
...
## Continuous Distributions: Sampling with R

\footnotesize

We can use R to sample from the logistic distribution.

```{r echo = TRUE, fig.height = 2, fig.width = 3}
rlogis(10)  # sample 10 times from std. logistic distribution
```
...
In many cases, the mass/density function will be "0 otherwise" for large regions of $[-\infty, \infty]$ and sum/integrate to zero over those regions. *Be sure to adjust the limits on your integrals accordingly* when computing the cdf.
...
Usually, it's easier to compute probabilities with the cdf than the pdf. Using the theorems above, we have the following tools to find $Pr(X \in C)$:

\begin{footnotesize}
\begin{center}
\begin{tabular}{cccc}
$C$ & Probability & Integral & Using the CDF \\
$[a, b]$ for $a < b$ & $\Pr(a \leq X \leq b)$ & $  \int_a^b f(x) dx$ & F(a) - F(b)\\
$[-\infty, b]$ & $\Pr(X \leq b)$ &$ \int_{-\infty}^b f(x) dx$ & F(b)\\
$[a, \infty]$ & $\Pr(X \geq a)$ & $ \int_a^\infty f(x) dx$ & 1 - F(a)\\
\end{tabular}
\end{center}
\end{footnotesize}
...
\begin{Exercise}
Suppose I'm going to sample once from a box with five tickets. The ticket are numbered with 1, 2, 3, 3, and 7. Write the pmf as a piecewise function. Sketch the cdf.
\end{Exercise}
...
Here's the intuition: Choose any error tolerance you like. There is a random sample large enough that the average of the sample will, \textit{for sure}, fall inside the tolerance.
...
\begin{Exercise}
Let $\overline{X}_n$ represent the average of $n$ draws (w/ replacement) from a hat with tickets numbered 1, 2, 3, 3, 3, and 8.  $\overline{X}_n$ converges in probability to what value? Confirm with a simulation.
\end{Exercise}
...
## Illustration of CLT's Convergence in Distribution
...
Roll a die $n$ times, treating sixes as 1 and not-sixes as 0. Compute the standardized sample average from the 10 rolls. Do this 10,000 times to get a good sense of the \textit{distribution} of the standardized sample average.
...
\begin{Exercise}
**Important**: Compare the formal ideas from the notes to FPP's box model from ch. 16-18. In what ways are they similar? In what ways are they different? Be especially sure to (A) compare drawing (once) from a box to a pmf/pdf and (B) compare the two versions of the central limit theorem. 
\end{Exercise}
